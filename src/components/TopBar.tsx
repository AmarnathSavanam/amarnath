...
export default function TopBar({ activeCategory, onCategoryChange, searchQuery, onSearchChange, onSearchClear, onLogoClick, compact }: TopBarProps) {
  ...
  return (
    <header
      className={`sticky top-0 z-40 -mx-4 sm:-mx-6 lg:mx-0 px-4 sm:px-6 lg:px-0 py-3 transition-all duration-300 ${
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <button
          onClick={onLogoClick}
          className="md:hidden font-display text-lg font-extrabold tracking-tight vapor-gradient-text"
        >
          STRM
        </button>
          ...
          {cats.map((c) => {
            ...
            return (
              <button
                ...
                className={`shrink-0 px-4 py-2 rounded-2xl text-[11px] sm:text-sm font-semibold tracking-wide transition-all duration-200 ${
                  active
