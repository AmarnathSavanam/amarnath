...
export default function HeroSpotlight({ item, onPlay, onMoreInfo, resumeItem, sideItem, onSideClick }: HeroSpotlightProps) {
  ...
  return (
    <section className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6 lg:gap-8 animate-fade-in">
      ...
      <div className="lg:col-span-8 relative rounded-[1.25rem] sm:rounded-[2.5rem] border border-white/10 overflow-hidden group shadow-2xl min-h-[300px] sm:min-h-[440px] lg:min-h-[520px]">
        <img
          src={images.banner}
          alt={item.title}
          className="absolute inset-0 w-full h-full object-cover transition-transform duration-[1200ms] group-hover:scale-105"
        />
        <div className="absolute inset-0 bg-gradient-to-t from-background via-background/70 sm:via-background/55 to-background/20 sm:to-transparent" />
        <div className="absolute inset-0 bg-gradient-to-r from-background/90 via-background/40 sm:via-background/20 to-transparent" />

        <div className="relative z-10 h-full flex flex-col justify-end p-5 sm:p-10 lg:p-14">
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
            <span className="px-2 py-1 vapor-gradient-bg text-background text-[10px] font-black uppercase rounded-md tracking-wider">#1 Trending</span>
            <span className="hidden sm:inline text-[10px] sm:text-xs font-medium tracking-[0.2em] uppercase" style={{ color: "hsl(var(--vapor-ice))" }}>
              Spotlight Selection
            </span>
          </div>

          <h1 className="font-display font-black tracking-tight leading-[0.9] mb-3 sm:mb-6 text-3xl sm:text-6xl lg:text-7xl xl:text-8xl">
          ...
          <div className="flex flex-wrap items-center gap-x-3 sm:gap-x-5 gap-y-2 mb-4 sm:mb-7 text-[11px] sm:text-sm text-foreground/60">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <button
              onClick={() => onPlay(item)}
              className="flex items-center gap-2 px-4 sm:px-8 py-2.5 sm:py-3.5 rounded-xl sm:rounded-2xl bg-foreground text-background font-bold text-xs sm:text-sm transition-all duration-200 hover:bg-primary hover:shadow-[0_0_30px_hsl(var(--vapor-cyan)/0.5)] active:scale-95"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 sm:px-7 py-2.5 sm:py-3.5 rounded-xl sm:rounded-2xl vapor-glass-strong text-foreground font-bold text-xs sm:text-sm hover:bg-white/10 transition-all active:scale-95"
            >
      ...
      <div className="lg:col-span-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4 sm:gap-6">
        {sideItem && sideImages && (
          <button
            ...
            className="group relative flex-1 rounded-[1.25rem] lg:rounded-[2rem] border border-white/10 p-5 sm:p-7 text-left overflow-hidden vapor-glass min-h-[150px] sm:min-h-[200px] lg:min-h-0"
          >
            ...
            <div className="relative">
              ...
              <h3 className="font-display text-lg sm:text-2xl font-bold mb-2 leading-tight line-clamp-2">{sideItem.title}</h3>
        ...
        {resumeItem && resumeImages && (
          <button
            ...
            className="relative rounded-[1.25rem] lg:rounded-[2rem] border border-white/10 p-5 sm:p-7 text-left vapor-glass flex flex-col justify-between min-h-[150px] sm:min-h-[200px] lg:min-h-0 group"
          >
            ...
            <div>
              <div className="font-display text-base sm:text-lg font-bold mb-1 line-clamp-1">{resumeItem.title}</div>
